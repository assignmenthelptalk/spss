...
The one-sample t-test compares your sample against a fixed, external number — not against another group in your dataset. The independent samples t-test compares two groups *within* your own data. Using a one-sample test when you actually have two groups to compare (or vice versa) produces a technically valid-looking result that answers the wrong question.
